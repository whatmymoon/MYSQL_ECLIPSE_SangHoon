...
-- replace(문자열, 기본문자열, 바꿀문자열) - 문자열안에 있는 기본 문자열을 바꿀 문자열로 대치
...
-- DATE_ADD(날짜, INTERVAL 기준값 더할 숫자) - 날짜에서 기준값 만큼 더한다.
-- ※ 기준값 : YEAR, MONTH, DAY, HOUR, MINUTE, SECOND
select date_add( now(), INTERVAL 3 YEAR) from dual;

-- DATE_SUB(날짜, INTERVAL 기준값 뺄 숫자) - 날짜에서 기준값 만큼 뺀다.
-- ※ 기준값 : YEAR, MONTH, DAY, HOUR, MINUTE, SECOND
...
-- DAYOFWEEK(날짜) - 날짜의 주 별 일자 출력(일요일(1), 월요일(2)...토요일(7))
